...
namespace Tutoring.MVC.Controllers
{
    ...
    public class CartController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            CartViewModel model = new CartViewModel
            {
                ...
                CartItems = cart.CartItems
                    ...
                    {
                        CartItemId = ci.Id,
                        LessonId = ci.Lesson.Id,
                        LessonName = ci.Lesson.Name,
                        LessonUrl = ci.Lesson.Url,
                        LessonImageUrl = ci.Lesson.ImageUrl,
                        Price = ci.Lesson.Price,
                        Amount = ci.Amount
                    }).ToList()
            };
            return View(model);
        ...
        {
            var userId = _userManager.GetUserId(User);
            await _cartManager.AddToCart(userId, id, amount);
            return RedirectToAction("Index");
        }
        [HttpPost]
        public async Task<IActionResult> ChangeAmount(int cartItemId, int amount)
        {
            if (ModelState.IsValid)
            {
                await _cartItemManager.ChangeAmountAsync(cartItemId, amount);
            }
            else
            {
        ...
        private Payment PaymentProcess(OrderViewModel model)
        {
            ...
            foreach (var item in model.Cart.CartItems)
            {
                ...
                basketItem.Name = item.LessonName;
                basketItem.Category1 = "Kitap";
                basketItem.ItemType = BasketItemType.PHYSICAL.ToString();
                basketItem.Price = Convert.ToInt32(item.Price * item.Amount).ToString();
        ...
        private async Task SaveOrder(OrderViewModel model, string userId)
        {
            ...
            foreach (var item in model.Cart.CartItems)
            {
                ...
                orderItem.LessonId= item.LessonId;
